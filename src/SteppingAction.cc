...
//  if(particlename=="neutron") G4cout<<logicname<<"\t"<<step->GetTrack()->GetKineticEnergy()/MeV<<"\t"<<step->GetTotalEnergyDeposit()<<"\t"<<location.x()<<"\t"<<location.y()<<"\t"<<location.z()<<G4endl;
...
//   G4cout<<projectile->GetParticleName()<<"\t"<<logicname<<G4endl;
  if(((projectile->GetParticleName()=="neutron")  && (logicname=="logictarget"))) {//Neutron flux mode
