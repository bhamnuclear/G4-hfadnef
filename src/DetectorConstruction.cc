...
void DetectorConstruction::SetFoilDistance(double val) {
   foildistance = val;
   G4cout<<"Setting foil distance to "<<foildistance/mm<<" mm"<<G4endl;
}

DetectorConstruction::DetectorConstruction() {
    G4cout << "DetectorConstruction constructed" << G4endl;
...
<------------------------------------------------------------->
*/
  G4double water_thickness = 0.3*mm;
  G4double li_thickness = 300*um;
  G4double cu_thickness = 3*mm-water_thickness;
  G4double cu_to_gr_gap = 20.5*mm;
...
//  G4cout<<"Return GDML world"<<G4endl;
     ...
     auto physWorld = new G4PVPlacement(nullptr,  // no rotation
      ...
      auto solidVac = new G4Box("Vacuum", 0.5 * env_sizeXY, 0.5 * env_sizeXY, 0.5*45*mm +  2*li_thickness);

      auto logicVac = new G4LogicalVolume(solidVac, vacuum, "Vacuum");
  ...
  G4ThreeVector VacPos = G4ThreeVector(0,vertical_offset,0.5*45*mm - 0.5*li_thickness);
  auto Vac = new G4PVPlacement(nullptr, VacPos, logicVac, "Vacuum", logicWorld, false, 0, checkOverlaps);
  ...
  auto logicEnv = new G4LogicalVolume(solidEnv,  // its solid
    ...
    "Lithium");                                 // its name
  G4cout<<"Lithium centre at "<<(0)/mm<<" mm away"<<G4endl;

  new G4PVPlacement(nullptr,  // no rotation
    G4ThreeVector(0*m,vertical_offset,0*m) - VacPos,          // at (0,0,0) is the centre of the lithium petal
...
//    logicWorld,               // its mother  volume
    ...
    checkOverlaps);           // overlaps checking
  //
  // Copper split into two
  //

  auto solidback = new G4Box("Copper",                    // its name
    0.5 * env_sizeXY, 0.5 * env_sizeXY, 0.5*0.5*cu_thickness);  // its size
...
//    copperwater,                                     // its material
  ...
  G4cout<<"Copper at 1 "<<(0.5*0.5*cu_thickness+0.5*li_thickness)/mm<<" mm away"<<G4endl;

  logicVac->SetVisAttributes(translucentcolour);
//First copper stage
  new G4PVPlacement(nullptr,  // no rotation
    G4ThreeVector(0,vertical_offset,0.5*0.5*cu_thickness+0.5*li_thickness) - VacPos,          //
...
//    logicWorld,               // its mother  volume
    ...
    checkOverlaps);           // overlaps checking
//Second copper stage
  G4cout<<"Copper at 1 "<<(0.5*1.5*cu_thickness+0.5*li_thickness+water_thickness)/mm<<" mm away"<<G4endl;

  new G4PVPlacement(nullptr,  // no rotation
    G4ThreeVector(0,vertical_offset,0.5*1.5*cu_thickness+0.5*li_thickness+water_thickness) - VacPos,          //
    logicback,                 // its logical volume
    "Copper",               // its name
//    logicWorld,               // its mother  volume
    logicVac,               // its mother  volume
  ...
  auto solidwat = new G4Box("Water",                    // its name
    0.5 * env_sizeXY, 0.5 * env_sizeXY, 0.5*water_thickness);  // its size

  auto logicwater = new G4LogicalVolume(solidwat,  // its solid
    water,                                     // its material
    "Water");
  auto watercolour = new G4VisAttributes();
  watercolour->SetColour(0.1,0.1,1.0);
  logicwater->SetVisAttributes(watercolour);

  new G4PVPlacement(nullptr,  // no rotation
    G4ThreeVector(0,vertical_offset,0.5*cu_thickness+0.5*li_thickness+0.5*water_thickness) - VacPos,          //
  ...
  G4cout<<"Graphite centre at 1 "<<(0.5*li_thickness+cu_thickness+water_thickness+cu_to_gr_gap+0.5*graphite1_thickness)/mm<<" mm away"<<G4endl;
  new G4PVPlacement(nullptr,  // no rotation
    G4ThreeVector(0,vertical_offset,0.5*li_thickness+cu_thickness+water_thickness+cu_to_gr_gap+0.5*graphite1_thickness) - VacPos,          // at (0,0,0)
...
//    logicWorld,               // its mother  volume
  ...
  G4cout<<"Graphite centre 2 at "<<(0.5*li_thickness+cu_thickness+water_thickness+cu_to_gr_gap+graphite1_thickness+g1_to_g2_gap+0.5*graphite2_thickness)/mm<<" mm away"<<G4endl;

  new G4PVPlacement(nullptr,  // no rotation
    G4ThreeVector(0,vertical_offset,0.5*li_thickness+cu_thickness+water_thickness+cu_to_gr_gap+graphite1_thickness+g1_to_g2_gap+0.5*graphite2_thickness) - VacPos,          // at (0,0,0)
...
//    logicWorld,               // its mother  volume
  ...
  new G4PVPlacement(nullptr,  // no rotation
    G4ThreeVector(0,vertical_offset,0.5*li_thickness+cu_thickness+water_thickness+cu_to_gr_gap+graphite1_thickness+g1_to_g2_gap+graphite2_thickness+g2_to_ti_gap+0.5*ti_thickness) - VacPos,
...
// Graphite thermal stack

  auto solidscatterer = new G4Box("scatterer",0.5*0.8*m,0.5*0.8*m,0.5*0.8*m);
  auto logicscatterer = new G4LogicalVolume(solidscatterer,graphite, "logic_thermalgraphite");
  new G4PVPlacement(0,G4ThreeVector(0,-0.7*m,-0.5*m),logicscatterer,"logic_thermalgraphite",logicWorld,false,0,checkOverlaps);
...
//  new G4PVPlacement(0,G4ThreeVector(0,0,0.046*m+55*mm+25*mm),logicscattererc,"logicscattererc",logicWorld,false,0,checkOverlaps);
  ...
  // Target space
  //
  G4double foil_size = 100*mm;
  G4double foil_thickness = 0.0001*mm;
  G4double ti_to_foil_gap = foildistance;//Get from macro (defaults to 0)
  G4double outer_distance = li_thickness+cu_thickness+water_thickness+cu_to_gr_gap+graphite1_thickness+g1_to_g2_gap+graphite2_thickness+g2_to_ti_gap+ti_thickness;
...
//  auto solidtarget = new G4Tubs("target", 0*mm, 6*mm, 0.14*mm, 0*deg, 360*deg);//Gold
 ...
 G4cout<<"Gap between Ti and foil is "<<ti_to_foil_gap/mm<<" mm"<<G4endl;
 G4cout<<"Target placed at "<<(outer_distance+ti_to_foil_gap+0.5*foil_thickness)/mm<<" mm from target"<<G4endl;
  new G4PVPlacement(nullptr,  // no rotation
//    G4ThreeVector(0,0,29*mm) - VacPos,          // at (0,0,0)
