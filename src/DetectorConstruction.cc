...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  // Envelope parameters
  //
  G4double env_sizeXY = 50*cm, env_sizeZ = 200*um;//Lithium target size
...
//    0.5 * 10 *cm, 0.5 * 10*cm, 0.5*1*mm);  // its size

  auto solidtarget = new G4Tubs("target", 0*mm, 70*mm, 10*mm, 0*deg, 360*deg);
...
/*
  new G4PVPlacement(nullptr,  // no rotation
    G4ThreeVector(0,0,0.046*m),          // at (0,0,0)
...
*/
  ...
  auto mesh = CADMesh::TessellatedMesh::FromSTL("standard_concrete.stl");
